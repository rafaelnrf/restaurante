...
function slidemenu(only_close) {
	...
	if ($('.slidemenu_btn').attr('data-slideopen') == "false" && !only_close) {
		...
		$('#slidemenu').anim({ translateX: '100%'}, 0.1, 'ease-out');

		$('#wrapper').anim({ translateX: '80%' }, 0.1, 'ease-out');
		
		$('.slidemenu_btn').attr('data-slideopen', "true");

	} else {

		$('#slidemenu').anim({ translateX: '-100%'}, 0.5, 'ease-out');
		$('#wrapper').anim({ translateX: '0px' }, 0.1, 'ease-out');
